...
(function() {
    ...
    function makeXHR(payload, successCb, errorCb) {
        ...
        switch(payload.method.toLowerCase()) {
            case 'post':
            case 'put':
                data = JSON.stringify(payload.data || data);
            case 'get':
     ...
     * @param {*} deducedHeaders - if any based on config
     * @param {*} payload - { method, url, headers, responseType, data, params }
     * @param {*} successCb - success callback
     * @param {*} errorCb - error callback
     */
    ...
    function execute(isAuth, api, payload, successCb, errorCb) {
        ...
        if (!endpoint) throw `No such API found - ${api} !!!`;
     ...
     */
    String.prototype.inject = function (item) {
    ...
    const lib = {
        ...
        getCustomRequestGroups : function(group, api) {
        ...
        executeCustom: function(group, api, payload, successCb, errorCb) {
            ...
            if (!group || !api)
                throw 'Group or API name cannot be undefined !!!';
            if (!customGroups[group])
                throw `No such group - "${group}" in custom request groups !!!`;

            const requestGroup = customGroups[group];
            const endpoints = requestGroup['endpoints'];
            if(!endpoints)
                throw `No endpoints in group - "${group}"`;

            const endpoint = endpoints[api];
            if (!endpoint)
                throw `No such api "${api}" in  group - "${group}" !!!`;

            const groupConfig = requestGroup.config || {};
        ...
        request : function(payload, successCb, errorCb) {
            if (!payload) throw 'Payload cannot be empty !!!';
            if (!payload.method) throw 'Method cannot be empty !!!';
            if (!payload.url) throw 'Url cannot be empty !!!';
            makeXHR(payload, successCb, errorCb);
        },
